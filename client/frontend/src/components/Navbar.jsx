...
const Navbar = () => {
  ...
  const sendVerificationOtp = async ()=>{
    try {
      ...
      // we will make the API call on the send verification OTP API endpoint that we have already created in our backend


      const {data} = await axios.post(backendUrl + '/api/auth/send-verify-otp')
    
      // mean the email is successfully sent that contains the verification OTP and after sending the OTP on the email we have to navigate user on the verify email page 
      if(data.success){
